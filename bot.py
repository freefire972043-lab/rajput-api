...
# ================== CONFIG ==================
BOT_TOKEN = "7965256274:AAGiPxfZiyDzfOOci9YPGD6tFCbWy93tcgQ"
OWNER_ID = 6820574331
...
def init_db():
    ...
    # Insert default API if not exists
...
# ================= AUTOLIKE SCHEDULER (NEW VERSION) =================
def run_autolike_task():
    try:
        ...
        for user in users:
            ...
            # Make the API call for the current user
            api_link = get_current_api()
            url = f"{api_link}/like?uid={uid}&server_name={region}&key=@adityaapis"
            response = make_request(url)
...
def autolike_scheduler():
    ...
    # Schedule the task to run every day at 04:00 India Time
...
if __name__ == "__main__":
    # Use a separate thread for Flask so it doesn't block the bot
